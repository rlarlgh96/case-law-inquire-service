...
따라서, 마약범죄 판례 조회 서비스는 마약범죄 형량에 관한 통계적 자료와 근거를 제공하기 위해 개발했다. 
마약범죄 판례 조회 서비스는 마약의 종류에 따라 선고된 형량 분포를 제공하고, 형량 범위에 판례를 제공한다.
 ...
 * 사이트로부터 7,259개의 판례를 수집했다.
 ...
 ### 2. 데이터 전처리
 * 수집한 판례는 비정형 데이터로, 이를 정형화하는 작업이 필요하다.
 * 판례를 형량 분포 그래프로 나타내기 위해 형 종류, 형량에 따라 분류하는 작업이 필요하다.ㅇ
