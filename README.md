...
또한, 마약범죄와 관련한 구체적인 데이터를 제공하는 기관은 현재 몇개의 판례 검색 사이트를 제외하면 국내에서 찾아보기 드물다.
따라서, 본 서비스는 이와 같은 사회적 분위기 속에서 출발했다.
본 서비스의 목적은 먀약범죄에 관한 형량 및 판례 데이터 제공에 있다.
마약범죄 판례 조회 서비스는 과거 마약에 관한 판례 데이터를 수집하고, 수집한 데이터를 바탕으로 사용자의 입력에 따라 판례 데이터의 형종별, 형량별 분포를 제공한다.
본 서비스는 형종, 형량 분포에 따라 판례 데이터를 제공하고 있기 때문에, 기존에 검색 키워드로 판례를 조회할 수 있는 다른 서비스보다 구체화된 결과를 제공한다.
또한, 특정 사건의 형량이 과거 판례들에 비해 적절했는지 본 서비스를 통해 확인해볼 수 있을 것이다.
 ...
 * 사이트에서 총 10가지의 마약의 이름을 키워드로 검색하여 판례 원문 데이터를 수집하였다.
 ...
 * 사용자가 선택한 마약(복수 선택 가능)에 따라 데이터 베이스 내 만족하는 데이터를 검색한다(데이터 베이스는 앞선 전처리 과정에서 생성한 data.csv 파일을 사용하였다).
...
## 사용 방법
 * 서비스 사용 방법은 다음과 같다.
 1) repository에 업로드된 파일 전체를 다운받는다.
 2) 개발환경에 맞는 가상환경을 실행한다.
 3) 터미널 경로를 myweb 폴더로 변경한다.
 4) 터미널에서 python manage.py runserver 명령을 실행한다.
 5) 브라우저를 통해 출력된 로컬 서버로 접속한다.
