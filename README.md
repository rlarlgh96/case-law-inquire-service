...
최근 정부에서는 "마약과의 전쟁"을 선포할만큼 이를 심각히 다루고 있다.
경찰청에 따르면 2018년 12,613명이었던 마약사범은 2021년 16,153명으로 크게 증가한 것으로 나타났다.
따라서, 본 서비스는 마약범죄 형량에 관한 통계적인 자료와 근거를 제공하기 위해 개발했다.
 ...
 * 판례 내용 구조를 분석해, 수집한 판례에서 필요한 정보만 추출하는 알고리즘을 만들어 데이터 전처리를 진행했다(데이터 전처리에 관한 자세한 내용은 preprocessing.ipynb 파일을 참고).
 * 데이터 전처리 결과, 수집한 7,259개의 데이터 중 3,162개의 데이터를 활용할 수 있는 형태로 가공했다.
 ...
 ### 3. 서비스 구현
 * Django를 사용해 웹사이트로 구상한 서비스를 구현했다.
