...
많은 언론에서도 마약혐의에 대해서만 집중적으로 보도할 뿐, 후에 어떤 처벌을 받았는지 또는 형량이 적절했는지에 대한 논의가 충분히 이루어지고 있지 않다.
따라서, 본 서비스는 이와 같은 사회적 분위기 속에서 출발했다.
본 서비스의 목적은 먀약범죄에 관한 형량 및 판례 데이터 제공에 있다.
마약범죄 형량 및 판례 조회 서비스는 과거 마약에 관한 판례 데이터를 수집하고, 수집한 데이터를 바탕으로 사용자의 입력에 따라 판례 데이터의 형종별, 형량별 분포를 제공한다.
본 서비스는 형량에 따른 판례 데이터를 제공하고 있기 때문에, 기존에 검색 키워드로 판례를 조회할 수 있는 다른 서비스보다 구체화된 결과를 제공한다.
또한, 특정 사건의 형량이 과거 판례들에 비해 적절했는지를 본 서비스를 통해 확인할 수 있다.
 ...
 * 전처리를 완료한 데이터 테이블은 다음과 같다.
 ...
 ### 3. 서비스 구현
 * 본 서비스는 Django 프레임워크를 사용해 웹사이트 형태로 구현하였다.
 ...
 * 사용자가 선택한 마약(복수 선택 가능)과 관련된 판례 데이터를 검색한다.
