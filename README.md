...
경찰청에 따르면 2018년 12,613명이었던 마약사범은 2022년 18,395명으로 크게 증가한 것으로 나타났다.
이처럼 급증하는 마약사범에도 불구하고, 실제 실형을 선고받는 비율은 오히려 감소해 논란이 일고 있다.
마약사범의 실형 비율은 2020년 53.7%에서 이듬해 50.6%로, 2022년엔 48.1%로 감소해왔다.
이에 일각에서는 마약 근절을 위해 마약사범 처벌 수위를 강화해야 한다는 목소리가 높아지고 있다.

마약범죄 판례 조회 서비스는 이러한 사회적 분위기 속에서 출발했다.
본 서비스의 목적은 마약범죄에 관한 보다 많은 정보 제공에 있다.
본 서비스는 사용자가 선택한 마약 종류에 따라 선고된 형량 분포와 판례를 제공하고 있어 기존의 다른 판례 조회 서비스보다 더 특화된 정보를 제공한다.
따라서 본 서비스를 통해 마약사범 처벌 현황을 파악하고 앞으로의 개선 방안에 관해 논의해볼 수 있을 것이다.
 ...
 * 수집한 7,259개의 데이터 중 3,162개의 데이터를 활용 가능한 형태로 처리했다.
 ...
 ### 3. 서비스 구현
 * Django를 사용해 서비스를 웹의 형태로 구현했다.
 * 사용자 입력값을 다음 페이지로 전송하기 위해 HTML form을 사용했다.
 * 사용자 입력값을 여러 페이지에서 걸쳐 사용하기 위해 세션 변수를 활용했다.
 * 서비스 구현을 위해 구성한 페이지는 다음과 같다.
 1) 홈페이지
 2) 검색 페이지: 사용자로부터 검색할 마약 종류를 입력받는 페이지
 3) 형종 데이터 분포 페이지: 입력값을 만족하는 데이터의 형종 분포를 나타내는 페이지
 4) 형량 데이터 분포 페이지: 입력값을 만족하는 데이터의 형량 분포를 나타내는 페이지
 5) 판례 조회 페이지
...
## 실행 방법
 * 이 서비스는 정식으로 배포하지 않았기에 로컬 환경에서만 실행이 가능하다.
 * 실행 방법은 다음과 같다.
 1) 업로드된 전체 파일을 다운받는다.
